    ...
    run: async (bot, message, args) => {
        ...
        if (!rMember) {
            let nopersonembed = new Discord.MessageEmbed()
                .setTitle("❌Error")
                .setDescription("Missing target user to remove role")
        ...
        if(rMember.roles.cache.has(role.id)) {
            await rMember.roles.remove(role.id).catch(O_o => {});

            let roleremoved = new Discord.MessageEmbed()
                .setDescription(`Role ${role.name} removed from ${rMember.displayName}`)
                .setColor(embedColor);
            await message.channel.send(roleremoved).then(msg => msg.delete({timeout: 5000})).catch(O_o => {});
        } else {
            let alreadyhaveroleEmbed = new Discord.MessageEmbed()
                .setTitle("❌Error")
                .setDescription("This user do not have the role")
                .setColor(warningColor);
