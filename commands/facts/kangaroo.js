...
module.exports = class kangarooCommand extends Commando.Command {
    constructor(client) {
        super(client, {
            name: 'kangaroo',
            group: 'facts',
            memberName: 'kangaroo',
            description: 'A image of kangaroo and a fact'
        });
    }
    ...
    async run(message, {sMESSAGE}) {
        ...
        const { fact, image } = await fetch(`https://some-random-api.ml/animal/koala?key=${SRAapi}`).then(response => response.json());
