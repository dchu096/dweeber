...
module.exports = class racoonCommand extends Commando.Command {
    constructor(client) {
        super(client, {
            name: 'racoon',
            group: 'facts',
            memberName: 'racoon',
            description: 'A image of racoon and a fact'
        });
    }
    ...
    async run(message, {sMESSAGE}) {
        ...
        const { fact, image } = await fetch(`https://some-random-api.ml/animal/racoon?key=${SRAapi}`).then(response => response.json());
