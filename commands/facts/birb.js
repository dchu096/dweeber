...
module.exports = class birbCommand extends Commando.Command {
    constructor(client) {
        super(client, {
            name: 'birb',
            group: 'facts',
            memberName: 'birb',
            description: 'A image of birb and a fact'
        });
    }
    ...
    async run(message, {sMESSAGE}) {
        ...
        const { fact, image } = await fetch(`https://some-random-api.ml/animal/birb?key=${SRAapi}`).then(response => response.json());
