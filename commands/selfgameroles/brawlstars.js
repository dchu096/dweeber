...
module.exports= {
    config: {
        name: "brawlstars",
        description: "Adds the role Brawl Stars to a yourself!",
        usage: " ",
    ...
    run: async (bot, message, args) => {
        ...
        const role = message.guild.roles.cache.find(r => r.id ===  '647303233483702282')
        if(!role) {
            let rolenotfoundEmbed = new Discord.MessageEmbed()
                .setTitle("❌Error")
                .setDescription("Roles not found on this server")
                .addField("required:", "Role Brawl Stars", false)
