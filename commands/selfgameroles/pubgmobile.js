...
module.exports= {
    config: {
        name: "pubgmobile",
        description: "Adds the role PUBG Mobile to a yourself!",
        usage: " ",
    ...
    run: async (bot, message, args) => {
        ...
        const role = message.guild.roles.cache.find(r => r.id ===  '647300483035430915')
        if(!role) {
            let rolenotfoundEmbed = new Discord.MessageEmbed()
                .setTitle("❌Error")
                .setDescription("Roles not found on this server")
                .addField("required:", "Role PUBG Mobile", false)
