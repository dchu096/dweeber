...
module.exports= {
    config: {
        ...
        description: "Adds the role Call Of Duty Mobile to a yourself!",
        usage: " ",
        category: "selfgameroles",
        accessableby: "Members",
        aliases: ["codm", "cod", "callofduty"]
    },
    run: async (bot, message, args) => {
        ...
        const role = message.guild.roles.cache.find(r => r.id ===  '647300351179096085')
        if(!role) {
            let rolenotfoundEmbed = new Discord.MessageEmbed()
                .setTitle("❌Error")
                .setDescription("Roles not found on this server")
                .addField("required:", "Role Call Of Duty Mobile", false)
