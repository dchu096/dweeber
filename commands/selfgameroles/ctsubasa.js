...
module.exports= {
    config: {
        name: "captaintsubasa",
        description: "Adds the role Captain Tsubasa to a yourself!",
        usage: " ",
    ...
    run: async (bot, message, args) => {
        ...
        const role = message.guild.roles.cache.find(r => r.id ===  '726328337450139704')
        if(!role) {
            let rolenotfoundEmbed = new Discord.MessageEmbed()
                .setTitle("❌Error")
                .setDescription("Roles not found on this server")
                .addField("required:", "Role Captain Tsubasa", false)
