...
module.exports= {
    config: {
        name: "exosheroes",
        description: "Adds the role Exos Heroes to a yourself!",
        usage: " ",
    ...
    run: async (bot, message, args) => {
        ...
        const role = message.guild.roles.cache.find(r => r.id ===  '726328419838722048')
        if(!role) {
            let rolenotfoundEmbed = new Discord.MessageEmbed()
                .setTitle("❌Error")
                .setDescription("Roles not found on this server")
                .addField("required:", "Role Exos Heroes", false)
