...
module.exports= {
    config: {
        name: "ragnarok",
        description: "Adds the role Ragnarok M to a yourself!",
        usage: " ",
        category: "selfgameroles",
        accessableby: "Members",
        aliases: ["ragnarokmobile"]
    },
    run: async (bot, message, args) => {
        ...
        const role = message.guild.roles.cache.find(r => r.id ===  '647303121273487372')
        if(!role) {
            let rolenotfoundEmbed = new Discord.MessageEmbed()
                .setTitle("❌Error")
                .setDescription("Roles not found on this server")
                .addField("required:", "Role Ragnarok M", false)
