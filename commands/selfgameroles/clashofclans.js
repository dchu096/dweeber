...
module.exports= {
    config: {
        name: "clashofclans",
        description: "Adds the role Clash Of Clans to a yourself!",
        usage: " ",
    ...
    run: async (bot, message, args) => {
        ...
        const role = message.guild.roles.cache.find(r => r.id ===  '647302463313149979')
        if(!role) {
            let rolenotfoundEmbed = new Discord.MessageEmbed()
                .setTitle("❌Error")
                .setDescription("Roles not found on this server")
                .addField("required:", "Role Clash Of Clans", false)
