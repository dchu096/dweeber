...
module.exports= {
  ...
  run: async (client, message, args, config, queue) => {
    ...
    if (!voiceChannel) return message.channel.send("You need to join a vc before you can access to this command")

    const permissions = voiceChannel.permissionsFor(message.guild.me)
    if (!permissions.has("CONNECT")) return message.channel.send("Missing connect permission for bot")
    if (!permissions.has("SPEAK")) return message.channel.send("Missing speak permission for bot")

    if (!args.length) return message.channel.send("You did not specify a song name/URL")
