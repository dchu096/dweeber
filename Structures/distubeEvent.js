...
module.exports = (client) => {
  try {
      ...
      const autoconnect = async () => {
        let guilds = [client.autoresume.keys()];
        ...
        for (const gId of guilds) {
          try {
            ...
            if (!guild) {
              ...
              signale.fatal(`Bot have been removed from guild: ${gId}`)
              continue;
            }
            ...
            if (!voiceChannel || !voiceChannel.members || voiceChannel.members.filter(m => !m.user.bot && !m.voice.deaf && !m.voice.selfDeaf).size < 1) {
              client.autoresume.delete(gId);
              signale.fatal(`No members in voice channel: ${data.voiceChannel}`)
              continue;
            }
            ...
            if (!textChannel) {
              client.autoresume.delete(gId);
            signale.fatal(`text channel has been removed: ${data.textChannel}`)
              continue;
            }
            let tracks = data.songs;
            if(!tracks || !tracks[0]){
                signale.fatal(`No songs in queue, so I'm deleting the autoresume data`)
              continue;
            }
            ...
            signale.success(`Added ${newQueue.songs.length} Tracks on the QUEUE and started playing ${newQueue.songs[0].name} in ${guild.name}`);
            ...
            signale.watch(`Changed autoresume track to queue adjustments + deleted the database entry`)
            if (!data.playing) {
              newQueue.pause();
            }
    ...
    client.distube
      ...
      .on(`searchResult`, (result) => {
        let i = 0
        interaction.followUp(`**Choose an option from below**\n${result.map((song) => `**${++i}**. ${song.name} - \`${song.formattedDuration}\``).join(`\n`)}\n*Enter anything else or wait 60 seconds to cancel*`)
      })
      // DisTubeOptions.searchSongs = true
      .on(`searchCancel`, interaction => interaction.followUp(`Searching canceled`).catch((e)=>console.log(e)))
      .on(`error`, (channel, e) => {
      ...
      .on(`searchNoResult`, interaction => interaction.followUp(`No result found!`).catch((e)=>console.log(e)))
